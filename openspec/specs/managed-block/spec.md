...
- **AND** a pipeline with metadata has been added since the last `wai init`
- **AND** the CLAUDE.md managed block does not include that pipeline
- **THEN** doctor reports warning: "CLAUDE.md managed block outdated — run 'wai init' to refresh"
...
- **AND** running `wai init` will overwrite the user's modifications
