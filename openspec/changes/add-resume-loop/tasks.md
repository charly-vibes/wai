...
- [ ] 1.1 After `create_handoff()` returns `Ok(handoff_path)`, compute the path
...
- [ ] 1.2 Write the relative path as UTF-8 text to
      `.wai/projects/<project>/.pending-resume`, overwriting if it exists

## 2. `wai prime` — resume detection helpers
- [ ] 2.1 Add `read_pending_resume(project_dir: &Path) -> Option<PathBuf>`:
...
- [ ] 2.2 Add `extract_next_steps(handoff_path: &Path) -> Vec<String>`:
...
- [ ] 3.1 In `prime::run()`, before the current handoff-snippet logic, call
...
- [ ] 3.2 If date equals today: render `⚡ RESUMING: {date} — '{snippet}'` then
...
- [ ] 3.3 If stale or absent: fall through to existing `• Handoff:` logic
      unchanged

## 4. `src/managed_block.rs` — autonomous loop section
- [ ] 4.1 In the `wai_block_content()` Rust string in `src/managed_block.rs`,
...
## 5. Refresh CLAUDE.md
- [ ] 5.1 Run `wai init --yes` in the repo root to regenerate the WAI:START
      block in `CLAUDE.md` with the new Autonomous Loop section
      (manual step; not a code deliverable)

## 6. Tests
- [ ] 6.1 Unit test: `close` writes `.pending-resume` with correct relative path
- [ ] 6.2 Unit test: second `close` call overwrites `.pending-resume`
- [ ] 6.3 Integration test: `prime` renders `⚡ RESUMING` block when
      `.pending-resume` present and handoff dated today; `prime` called twice
      in succession still shows RESUMING (signal not consumed)
- [ ] 6.4 Integration test: `prime` renders normal `• Handoff:` line when
      `.pending-resume` handoff is dated yesterday (stale signal ignored)
- [ ] 6.5 Integration test: `prime` renders normally when `.pending-resume`
      is absent (no regression on existing behaviour)
- [ ] 6.6 Integration test: `prime` resume block with empty `## Next Steps`
      section (or only HTML comments) shows only the header line, no items
- [ ] 6.7 End-to-end loop test: run `close` → verify `.pending-resume` written →
