...
- [x] **[agnostic-capabilities-16]** Migrate plugin loader in `src/plugin.rs` from YAML (`*.yml`/`*.yaml`) to TOML (`*.toml`): change extension filter and swap `serde_yml::from_str` for `toml::from_str`.
- [x] **[agnostic-capabilities-17]** Add integration test `way_verbose_shows_intent`: run `wai way -v` and assert output contains `"Intent:"`.
- [x] **[agnostic-capabilities-18]** Add integration test `way_verbose_shows_success_criteria`: run `wai way -v` and assert output contains `"Success:"`.
