...
- [x] 2.1 Add `write_reflect_resource(project_root, project_name, content, handoff_count)`
  ...
  available in run())
- [x] 2.2 Add suffix logic: if `<date>-<project>.md` already exists, try `-2`, `-3`, etc.
...
- [x] 2.3a Add YAML front-matter writer: `date`, `project`, `sessions_analyzed` (=
...
- [x] 4.5 Update this repo's own `CLAUDE.md` by running `wai init` (or manually) after
  implementing 4.1–4.4

## Phase 5: Gather Context from Resource Files

- [x] 5.1 Extend `gather_reflect_context()` to also read
...
- [x] 5.2 Label this tier in the LLM prompt: "Previous reflections (extend and correct,
  do not repeat)"
- [x] 5.3 Cap at ~20K chars (newest files first)

## Phase 6: Tests

- [x] 6.1 Unit test: `write_reflect_resource()` writes to correct path with front-matter
- [x] 6.2 Unit test: suffix logic (`-2`, `-3`) when file already exists
- [x] 6.3 Unit test: `wai_reflect_ref_content()` contains "wai search" and the resource path
- [x] 6.4 Unit test: search-before-research instruction present with companions, absent without
- [x] 6.5 Unit test: `WAI:REFLECT:REF:START/END` block injected by `inject_managed_block()`
- [x] 6.6 Integration test: migration path — old block detected → resource file created →
  old block replaced with REF block
- [x] 6.7 `cargo check` passes with no warnings
- [x] 6.8 `cargo test` passes
