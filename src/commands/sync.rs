...
fn execute_symlink(project_root: &Path, config_dir: &Path, proj: &Projection) -> Result<()> {
    require_safe_mode("sync agent configs")?;
...
fn execute_inline(project_root: &Path, config_dir: &Path, proj: &Projection) -> Result<()> {
    require_safe_mode("sync agent configs")?;
    let target = project_root.join(&proj.target);
...
fn execute_reference(project_root: &Path, config_dir: &Path, proj: &Projection) -> Result<()> {
    require_safe_mode("sync agent configs")?;
    let target = project_root.join(&proj.target);
    let mut content = String::from("# Auto-generated by wai — do not edit directly\n");
